...
The stored model remains in body-scale units. The loader adapter applies one
global scale and translation derived from the complete 48-frame bounds before
returning the existing canvas schema. This display-only fit preserves all
relative motion and jump height and avoids changing the renderer or applying
...
The minimum three-clip activation gate is evaluated after all filters. A clip
must retain at least 35% effective landmark coverage to count toward its view;
a downloaded clip whose observations are entirely masked can therefore never
satisfy the front/side quorum.

For optimizer seeding, a frame/landmark with only one yaw group uses a
minimum-norm provisional X/Z value. Coordinates are then interpolated from the
nearest phase-aligned frames that have a full-rank yaw solution. If an entire
landmark lacks a second yaw, its provisional single-view seed is retained at
low confidence rather than aborting the complete build.

## Sparse bundle adjustment

The optimization variable is `P[48,33,3]`. Player-specific canonical major-bone lengths are initialized from the median of high-confidence reconstructed frames. When one major limb has no full-rank observation for the entire timeline, its left/right counterpart provides a conservative bone-length prior; this keeps the build numerically stable without inventing per-frame depth observations.
...
The reconstruction core, offline Curry builder, validation gates, model loader,
and API fallback integration are implemented. Synthetic orthogonal-view tests
confirm exact noiseless XYZ recovery, median robustness, fixed-scale vertical
motion preservation, projection consistency, temporal outlier rejection, and
sparse optimization of projection/bone/continuity residuals.

The real Curry model was not activated in this environment. All four attempted
front sources returned a 195-byte `Site Unavailable` HTML response in place of
video media, despite yt-dlp completing successfully. Repository history was
also audited: the deleted front entry contains three angle-only frames and no
33-landmark observations, so it cannot be used as reconstruction input. The
builder now rejects non-video download results, the failed build is recorded in
`models/curry_canonical3d_validation.json`, and the endpoint safely retains the
legacy Curry skeleton until a build with at least three accepted clips per view
passes every gate.

No set-point detector was added in this iteration. Existing catch/dip/rise/
release/follow-through anchors are reused exactly; the schema and resampling
module remain open to a later explicit set-point anchor without changing the
runtime contract.

## Paris 2024 USA roster extension

The same validated model contract is now enabled for the five Paris 2024 USA
players present in the repository: Stephen Curry, Devin Booker, Kevin Durant,
Anthony Edwards, and LeBron James. The offline batch processes and publishes
each player independently. A failed player therefore retains the legacy
skeleton without blocking or overwriting another validated player.

Source selection now requires the player's distinctive surname in video
metadata before pose analysis. This excludes an existing Anthony Davis clip
that had incorrectly entered Anthony Edwards' candidate pool through the
shared first name. The actual five-player build attempt is recorded in
`models/usa_olympic_canonical3d_validation.json`; the current environment
returned no valid YouTube media files, so no unvalidated model was activated.
